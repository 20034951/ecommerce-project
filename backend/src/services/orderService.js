...
const { Order, OrderItem, OrderStatusHistory, User, UserAddress, ShippingMethod, Coupon, Product } = db;
...
class OrderService {
    ...
    async createOrder(orderData, userId) {
        ...
        try {
            const { addressId, items, shippingMethodId, couponId, totalAmount } = orderData;
            ...
            if (!address) {
                throw new HttpError(404, 'Dirección no encontrada o no pertenece al usuario');
            }
    ...
    async getAllOrders(filters = {}) {
        const { status, page = 1, limit = 10, search, userId, startDate, endDate } = filters;
        const offset = (page - 1) * limit;
    ...
    async exportOrdersToCSV(filters = {}) {
        ...
        const rows = orders.map(order => {
            ...
            return [
                ...
                order.address ? `${order.address.address_line1}, ${order.address.city}` : 'N/A'
            ];
        });
