...
export default function ProductCard({ product }) {
  ...
  // Obtener color de la categoría o usar el predeterminado
  const categoryColor = product?.category?.color || "#6366f1";
  const categoryEmoji = product?.category?.emoji;

  return (
    <div
      className="group relative bg-white dark:bg-gray-800 rounded-xl border overflow-hidden shadow-sm hover:shadow-xl dark:hover:shadow-gray-900/50 transition-all duration-300 cursor-pointer flex flex-col h-full"
      style={{
        borderColor: `${categoryColor}20`,
      }}
      onMouseEnter={(e) => {
        e.currentTarget.style.borderColor = categoryColor;
        e.currentTarget.style.boxShadow = `0 20px 25px -5px ${categoryColor}20, 0 10px 10px -5px ${categoryColor}10`;
      }}
      onMouseLeave={(e) => {
    ...
    >
      ...
      <div className="relative overflow-hidden bg-gray-100 dark:bg-gray-700 aspect-square">
        ...
        {stockInfo && (
          <div
            className={`absolute top-3 left-3 px-3 py-1 rounded-full text-xs font-semibold ${stockInfo.color} shadow-sm`}
          >
            {stockInfo.text}
          </div>
        )}
        ...
        <button
          ...
          className={`absolute bottom-3 right-3 p-3 rounded-full shadow-lg transition-all duration-300 text-white transform group-hover:scale-110
                        ${
                          product.stock === 0
          ...
          style={
            product.stock !== 0
              ? {
                  ...
                  boxShadow: `0 10px 15px -3px ${categoryColor}50`,
                }
              : {}
          }
          onMouseEnter={(e) => {
      ...
      <div className="p-4 flex flex-col flex-grow">
        ...
        {product?.category?.name && (
          <div
            className="flex items-center gap-1.5 text-xs font-medium mb-2 px-2 py-1 rounded-md w-fit transition-all duration-200"
            style={{
        ...
        {/* Nombre del producto */}
        <h2
          className="font-semibold text-base lg:text-lg text-gray-900 dark:text-white mb-2 line-clamp-2 transition-colors duration-200"
          onMouseEnter={(e) => {
